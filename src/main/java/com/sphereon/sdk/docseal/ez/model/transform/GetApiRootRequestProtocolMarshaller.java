...
public class GetApiRootRequestProtocolMarshaller implements Marshaller<Request<GetApiRootRequest>, GetApiRootRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.API_GATEWAY).requestUri("/docseal/v1/ez/")
            .httpMethodName(HttpMethodName.GET).hasExplicitPayloadMember(false).hasPayloadMembers(false).serviceName("EzDocSeal").build();
